...
def get_model(args: Namespace) -> nn.Module:
    ...
    if args.impl == "ours":
        return PolyGCL(in_size=args.in_dim, hidden_size=args.hid_dim, out_size=args.hid_dim, K=args.K, dropout_p=args.dprate)
...
def get_kwargs(cls, args: Namespace) -> dict:
    ...
    assert set(kwargs.keys()) == set(init_args), f"GOT: {set(kwargs.keys())} - EXPECTED: {set(init_args)}"
    return kwargs
